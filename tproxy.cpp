...
tproxy::tproxy(nat *n) {
    ...
    setsockopt(this->ep_param.fd, SOL_IP, IP_TRANSPARENT, &opt, sizeof(opt));
    setsockopt(this->ep_param.fd, SOL_IP, IP_RECVORIGDSTADDR, &opt, sizeof(opt));
    set_nonblock(this->ep_param.fd);
    struct sockaddr_in serv{};
    serv.sin_family = AF_INET;
    serv.sin_addr.s_addr = htonl(INADDR_ANY);
    serv.sin_port = htons(n->config.tproxy_port);
    bind(this->ep_param.fd, (const struct sockaddr *) &serv, sizeof(serv));
